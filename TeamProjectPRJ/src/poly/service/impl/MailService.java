...
public class MailService implements IMailService {
	...
	final String user = "@naver.com"; // 본인 네이버 아이디
	...
	public int doSendMail(MailDTO pDTO) {
		...
		//전달 받은 DTO로부터 데이터 가져오기(DTO객체가 메모리에 올라가지 않아 Null이 발생할 수 있기 때문에 에러 방지차원으로 if문 사용)
		if(pDTO == null) {
			pDTO = new MailDTO();
		}
		...
		props.put("mail.smtp.auth", "true"); // javax 외부 라이브러리에 매일 보내는 사람 인증 여부 설정
